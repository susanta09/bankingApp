...
<div><jsp:include page="adminNab.jsp"></jsp:include></div>
	<div>
		<form action="/MyShop/searchtran/">
			<input type="text" name="rid" value="${rid}" placeholder=" Registration number">
			<button>Search</button>
			<a class="a" href="/MyShop/searchT">Refresh</a>
		</form>
	</div>
	<div>
		<div>
					...
					<c:forEach begin="1" end="${noP}" var="i">
						<c:choose>
							<c:when test="${currentP eq i}">
								<td><h1>
								<a href="/MyShop/tran/${i}/${rid}" style="text-decoration: none;">p${i}</a>
								</h1></td>
							</c:when>
							<c:otherwise>
								<td><h1>
